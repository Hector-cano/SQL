...
SELECT * from Purchasing.PurchaseOrderHeader where Status = 4 and Subtotal > 10.000;
...
SELECT PurchaseOrderID from Purchasing.PurchaseOrderHeader Where (totaldue between 1000 and 2000);
...
SELECT * from Purchasing.PurchaseOrderHeader WHERE '2012-01-01' < OrderDate;
...
SELECT PurchaseOrderID,Freight from Purchasing.PurchaseOrderHeader WHERE Freight > (0.01 * SubTotal);

-- 07 �Cu�les son las filas en las que el EmployeeID est� en el rango de 250 a 260?

SELECT EmployeeID from Purchasing.PurchaseOrderHeader WHERE (EmployeeID between 250 and 260);

-- 08 �Qu� registros tienen un PurchaseOrderID igual a 9 o 10?

SELECT PurchaseOrderID from Purchasing.PurchaseOrderHeader Where PurchaseOrderID = 9 or PurchaseOrderID = 10;
SELECT PurchaseOrderID from Purchasing.PurchaseOrderHeader Where PurchaseOrderID in (9, 10);

-- 09 �Qu� filas tienen un ShipDate exactamente igual a la fecha de ModifiedDate?

SELECT ShipDate,ModifiedDate from Purchasing.PurchaseOrderHeader Where ShipDate = ModifiedDate;

-- 10 �Qu� filas tienen un SubTotal mayor o igual a 50,000 y un Freight menor o igual a 2,000?

SELECT SubTotal,Freight FROM Purchasing.PurchaseOrderHeader WHERE SubTotal >= 50.000 and Freight <= 2.000;

-- 11 �Qu� filas tienen un TaxAmt superior a 10 y un TotalDue inferior a 100?

SELECT TaxAmt, TotalDue FROM Purchasing.PurchaseOrderHeader WHERE TaxAmt > 10 and TotalDue < 100;

-- 12 �Qu� filas tienen un Status que no es 4?

SELECT Status FROM Purchasing.PurchaseOrderHeader WHERE Status <> 4;

-- 13 �Cu�les son las �rdenes donde ShipMethodID comienza por el n�mero 2 (usa LIKE)?

SELECT ShipMethodID FROM Purchasing.PurchaseOrderHeader WHERE ShipMethodID like 2;

-- 14 �Qu� registros tienen un OrderDate en 2011?

SELECT OrderDate From Purchasing.PurchaseOrderHeader WHERE '2011-01-01' <=OrderDate and Orderdate<='2011-12-31';

-- 15 �Qu� filas tienen un SubTotal igual a la suma de TaxAmt y Freight?

SELECT SubTotal FROM Purchasing.PurchaseOrderHeader WHERE SubTotal = TaxAmt + Freight;
...
SELECT PurchaseOrderID FROM Purchasing.PurchaseOrderHeader WHERE PurchaseOrderID > 50 or TotalDue < 500;

-- 19 �Qu� registros tienen un TaxAmt superior a 1 y un Freight entre 10 y 20?

SELECT TaxAmt,Freight FROM Purchasing.PurchaseOrderHeader WHERE TaxAmt > 1 and (Freight between 10 and 20);

-- 20 �Qu� filas tienen un Status que sea 1, 3 o 5?

SELECT Status FROM Purchasing.PurchaseOrderHeader WHERE Status in (1,3,5);
...
select ShipDate, OrderDate, cast(ShipDate- OrderDate as int) as days from Purchasing.PurchaseOrderHeader where ShipDate- OrderDate > 10;

select ShipDate, OrderDate, cast(ShipDate- OrderDate as int) as days from Purchasing.PurchaseOrderHeader order by days desc;